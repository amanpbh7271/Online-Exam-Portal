...
for($i=1;$i<=$num;$i++)
{
 $q="update quiz set markoption='$answer[$i]' where id='$i'";
 mysqli_query($con,$q); 
}
 ...
 $em= $_SESSION["email"];
  ...
  for($i=1;$i<=$num;$i++)
{
 $q1="update result set $em1='$answer[$i]' where id='$i'";
 mysqli_query($con,$q1); 
}
...
//for right and wrong qus
  for($i=1;$i<=$num;$i++)
  {
    ...
    if($row['markoption']==$row['rightoption'])
    {
      $rightques++;
	  $forcheack[$i]=2;     
	}
	 else if($row['markoption']=='0'){
		 $forcheack[$i]=0;
		 $notattempt++;
      }
      else{
...
.red{
  background-color:Tomato;
   ...
   <div class="row" >
           <div class="col-sm-3">  
					<div >
					<h1 >Thanks <?php  echo $_SESSION["names1"]; ?></h1>
					</div>
           </div>
		   <div class="col-sm-6">  
					<div >
	   ...
	   <?php 
			    for($i=1;$i<=$num;$i++)
				{
					...
					$row=mysqli_fetch_array($result);
			    ?>
			 <div style="background:white; border:solid; ">			
			  <div id="ques<?php echo $i;?>" >	
			  <div>
		<?php if($forcheack[$i]==0)	{ ?> <label id="q" for="questions"   > <p  style="background-color:yellow;" id="ques<?php echo $i; ?>"><?php echo $i; echo $row['qus'];?></p></label>
		<?php } ?>
